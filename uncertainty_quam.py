...
#         total = - torch.mean(
#             torch.sum(
#                 (average_net_pred.unsqueeze(1).unsqueeze(2) * torch.log(sample_preds + gamma)), dim=3), # .unsqueeze(2)
#             dim=(1, 2))
#         aleatoric = - torch.sum( (average_net_pred+gamma) * torch.log(average_net_pred+gamma), dim=1)
#         epistemic = total - aleatoric
#     else:
# #         # ce_sm: [B, S, M, H, W]
# #         w = sample_weights.unsqueeze(-1).unsqueeze(-1)        # [B, S, M, 1, 1]
# #         w = w / (w.sum(dim=(1,2), keepdim=True) + 1e-8)       # safety
# # #         total = (ce_sm * w).sum(dim=(1, 2))                   # [B, H, W]
# #         print('sample_weights',sample_weights.shape)
# #         print('average_net_pred',average_net_pred.shape)
# #         print('sample_preds', sample_preds.shape)

#         # sample weight implied to be normalized
#         total = - torch.sum(
#             torch.sum(
#                 (average_net_pred.unsqueeze(1).unsqueeze(2) * torch.log(sample_preds + gamma)), dim=3)*sample_weights.unsqueeze(-1).unsqueeze(-1),
#             dim=(1, 2))
#         aleatoric = - torch.sum( (average_net_pred+gamma) * torch.log(average_net_pred+gamma), dim=1)
...
# #     if sample_weights is None:
# #         print('sample_preds',sample_preds.shape)
# #         print('torch.mean(sample_preds, dim=(0, 1)',torch.mean(sample_preds, dim=(1,2)).shape)
# #         print('torch.mean(sample_preds, dim=(1, 2)) * torch.log(torch.mean(sample_preds, dim=(1, 2))',(torch.mean(sample_preds, dim=(1, 2)) * torch.log(torch.mean(sample_preds, dim=(1, 2)))).shape)
# # #               print('torch.sum(torch.mean(sample_preds, dim=(0, 1))',torch.sum(torch.mean(sample_preds, dim=(0, 1)).shape)
              
# #         total = - torch.sum(torch.mean(sample_preds, dim=(1, 2)) * torch.log(torch.mean(sample_preds, dim=(1, 2))), dim=1)
# #         print('total',total.shape)
# #         print('sample_preds * torch.log(sample_preds),',(sample_preds * torch.log(sample_preds)).shape)
# #         aleatoric = - torch.mean(torch.sum(sample_preds * torch.log(sample_preds), dim=3), dim=(1,2))
# #         print('aleatoric',aleatoric.shape)
# #         epistemic = total - aleatoric
# #     else:
...
# ):
#     eps = 1e-8
#     sample_preds = torch.clamp(sample_preds, eps, 1.0)  # to prevent log(0)

#     B, Nsamples, Nmodels, C, H, W = sample_preds.shape
#     total_samples = Nsamples * Nmodels
#     sample_preds_merged = sample_preds.view(B, total_samples, C, H, W)

#     if sample_weights is not None:
#         sample_weights = sample_weights.view(B, total_samples)  # [B, total_samples]
#         sample_weights = sample_weights / (sample_weights.sum(dim=1, keepdim=True) + eps)  # normalize weights
# #         print('sample_weights',sample_weights.shape)
# #         print('average_net_pred',average_net_pred.shape)
# #         print('sample_preds', sample_preds.shape)
...
#         # Total entropy of the weighted mean
#         total = - torch.sum(mean_pred * torch.log(mean_pred), dim=1)  # [B, H, W]

#         # Aleatoric: average entropy across samples, weighted
#         entropy_per_sample = - sample_preds_merged * torch.log(sample_preds_merged)  # [B, N, C, H, W]
#         weighted_entropy = entropy_per_sample * sample_weights[:, :, None, None, None]
#         aleatoric = weighted_entropy.sum(dim=1).sum(dim=1)  # [B, H, W]
...
# --- UNCERTAINTY CALCULATION (MIS Implementation) ---
