...
class SegNet(nn.Module):
    ...
    def _init_vgg16_encoder(self):
        vgg = vgg16(weights=VGG16_Weights.DEFAULT)
        # Extract Conv2d layers' state_dict
        params = [m.state_dict() for m in vgg.features if isinstance(m, nn.Conv2d)]

        idx = 0
        for name, module in self.named_modules():
            if isinstance(module, nn.Conv2d) and name.startswith("encoder"):
                if idx == 0:
                    # The first layer that normally expects 3 channels
                    w_3ch = params[idx]['weight']  # [64, 3, 3, 3]
                    b_3ch = params[idx]['bias']  # [64]

                    # Collapse 3 in-channels to 1 by taking the mean
                    w_1ch = w_3ch.mean(dim=1, keepdim=True)  # [64, 1, 3, 3]

                    with torch.no_grad():
                        module.weight.copy_(w_1ch)
                        module.bias.copy_(b_3ch)
                    idx += 1
                else:
                    module.load_state_dict(params[idx])
                    idx += 1
